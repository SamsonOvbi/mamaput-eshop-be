...
bookRoute.get('/', asyncHandler(async (req, res) => {
...
bookRoute.get('/paged', asyncHandler(async (req, res) => {
  const pageSize = 3;
...
bookRoute.get('/categories', asyncHandler(async (req, res) => {
  const categories = await BookModel.find().distinct('category');
  res.send(categories);
})
);

bookRoute.get('/seed', asyncHandler(async (req, res) => {
...
bookRoute.get('/slug/:slug', asyncHandler(async (req, res) => {
...
bookRoute.get('/:id', asyncHandler(async (req, res) => {
...
bookRoute.post('/', isAuth, isAdmin, asyncHandler(async (req, res) => {
...
bookRoute.put('/:id', isAuth, isAdmin, asyncHandler(async (req, res) => {
...
bookRoute.delete('/:id', isAuth, isAdmin, asyncHandler(async (req, res) => {
  const book = await BookModel.findOne(req.params.id);
  if (book) {
...
bookRoute.post('/:id/reviews', isAuth, asyncHandler(async (req, res) => {
  const bookId = req.params.id;
  const book = await BookModel.findOne(bookId);
  if (book) {
